...
SELECT regexp_replace(sv.ho || ' ' || sv.dem || ' ' || sv.ten, '  ',' ') as hovaten, sv.code, sv.ma_lop_hanh_chinh, at.so_tiet_vang, lp.id as lop_mon_hoc_id, en.id as enrollment_id, sv.encoded_position as encoded_position, lt.tuan
  FROM attendances at
  inner join lich_trinh_giang_days lt on at.lich_trinh_giang_day_id=lt.id
  inner join lop_mon_hocs lp on lp.id = lt.lop_mon_hoc_id
  inner join sinh_viens sv on at.sinh_vien_id=sv.id
  inner join enrollments en on en.sinh_vien_id=sv.id and en.lop_mon_hoc_id=lp.id
