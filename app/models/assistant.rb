class Assistant < ActiveRecord::Base
 # attr_accessible :lop_mon_hoc_id, :user_id, :giang_vien_id, :trogiang
  ...
  def get_lichs
  	lop_mon_hoc.lich_trinh_giang_days.includes_vi_pham.with_giang_vien(giang_vien.id)
  end
  ...
  def khoi_luong_du_kien
    self.lop_mon_hoc.lich_trinh_giang_days.where(giang_vien_id: self.giang_vien.id).accepted_or_completed.sum(:so_tiet)
  end
end
