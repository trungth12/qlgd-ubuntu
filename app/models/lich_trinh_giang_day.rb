...
#  attr_accessible :lop_mon_hoc_id, :moderator_id, :noi_dung, :phong, :so_tiet, :state, :thoi_gian, :thuc_hanh, :tiet_bat_dau, :tiet_nghi, :tuan, :status, :giang_vien_id, :so_tiet_moi, :note, :ltype
  ...
  scope :completed, -> {where(status: :completed,state:["bosung","normal"])}
  ...
  TIET = {[7,00] => 1, [7,55] => 2, [8,55] => 3,
    [9,50] => 4, [10,45] => 5, [13,00] => 6,
    [13,55] => 7, [14,55] => 8, [15,50] => 9,
    [16,45] => 10, [17,55] => 11, [18, 45] => 12,
    [19, 0] => 13, [20,50] => 14, [21,55] => 15}
  
  TIET2 = {1 => [7,00], 2 => [7,55], 3 => [8,55],
    4 => [9,50], 5 => [10,45], 6 => [13,00],
    7 => [13,55], 8 => [14,55], 9 => [15,50],
    10 => [16,45], 11 => [17,55], 12 => [18, 45],
    13 => [19,00], 14 => [20,50], 15 => [21,55]}
  CA = {1 => Time.strptime('7:00','%H:%M'), 2 => Time.strptime('9:49','%H:%M'), 3 => Time.strptime('12:59','%H:%M'), 4 => Time.strptime('14:54','%H:%M'), 5 => Time.strptime('17:54','%H:%M'), 6 => Time.strptime('20:49','%H:%M')}
  ...
  def conflict?(lich)
   ...
   # if thoi_gian < lich.thoi_gian
    #  return (lich.tiet_bat_dau > tiet_bat_dau) and (lich.tiet_bat_dau < tiet_bat_dau + so_tiet)
    #else
     # return (tiet_bat_dau > lich.tiet_bat_dau) and (tiet_bat_dau < lich.tiet_bat_dau + lich.so_tiet)
    #end 
    return (thoi_gian < lich.thoi_gian ? ( (lich.tiet_bat_dau > tiet_bat_dau) and (lich.tiet_bat_dau < tiet_bat_dau + so_tiet) ): ( (tiet_bat_dau > lich.tiet_bat_dau) and (tiet_bat_dau < lich.tiet_bat_dau + lich.so_tiet) ) )
  end
  ...
  def self.with_giang_vien(giang_vien_id)
   self.where(giang_vien_id: giang_vien_id)
  end
