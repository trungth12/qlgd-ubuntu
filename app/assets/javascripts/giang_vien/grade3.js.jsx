	...
	calDCC: function(){
		$.ajax({
          ...
          success: function(data) {                      
            ...
            React.unmountAndReleaseReactRootNode(document.getElementById('grades2'));
            React.renderComponent(<GroupGrade lop={this.props.lop} />, document.getElementById('grades2'));
			alert("Đã tính xong điểm chuyên cần!");
          }.bind(this)
        }); 
	},
	hoanthanh: function(){
		$.ajax({
          url: "/teacher/lop/"+this.props.lop+"/finish.json",
          type: 'POST',
          success: function(data) {                      
            if (data.result === 'OK') {
				alert("Lớp học hoàn thành");
		...
		return (
			<div>
				<table class="table table-bordered">
					<tr>
					<td><button class="btn btn-sm btn-primary" onClick={this.calDCC}>Tính điểm chuyên cần (cột điểm đầu tiên)</button></td>
					<td><button class="btn btn-sm btn-primary" onClick={this.hoanthanh}>Hoàn thành lớp học (khoá lớp: không sửa điểm được)</button></td>
					</tr>	
				</table>
...
var DisabledGradeCell = React.createClass({
	render: function(){
		var t = this.props.data.name;
		var x = null;
		if (t == "Điểm chuyên cần") {
			x = <span></span>;
		} else {
			x = <button class="btn btn-sm btn-primary" onClick={this.props.onEdit}>Sửa</button>;
		}	
		return (
