...
var DisabledGradeCell = React.createClass({	
	...
	render: function(){	
		if (this.state.hover === 0){
			return (	
				<td>
					<div onMouseEnter={this.onMouseEnter} onMouseLeave={this.onMouseLeave}>{this.props.data.grade === 0 ? '' : this.props.data.grade}</div>
				</td>
			)	
		} else {
			return (
				<td><div id="mainbtn" onMouseEnter={this.onMouseEnter} onMouseLeave={this.onMouseLeave}>{this.props.data.grade === 0 ? '' : this.props.data.grade}
						<button class="btn btn-sm btn-primary" onClick={this.props.onEdit}>Sửa</button>
					</div>
				</td>				
			)
		}
